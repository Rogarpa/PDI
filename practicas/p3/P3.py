...
class Filters:
    ...
    def mosaicHtmlTagger(self,sqOgWidth,sqOgLength, array, width, length, pixelTagFunction, header, footer):
        ...
        for m in range(rowsOfSquares):
            with open('result.html', 'a') as f:
                f.write("<tr><td><nobr>")
            self.update_string.set("m's color filter:"+str(int((m/rowsOfSquares)*100)) + '/100%')
    ...
    def mosaicHtmlTagger_with_context(self,sqOgWidth,sqOgLength, array, width, length, pixelTagFunction, header, footer):
        ...
        with open('result.html', 'a') as f:
                    f.write(footer)
        self.update_string.set('100/100')
    # def tagtester(self, array, width, length):
    #     tagger = lambda r,g,b: (
    #         "<span style=\"color:#{:02x}{:02x}{:02x};\">@</span>".format(r, g, b)
    #     )
                    
    #     self.mosaicHtmlTagger(10,10,array, width, length, tagger)
    # falta arreglar repeat_letter
...
class Aplicacion(Frame):
    ...
    def create_toolbar_widgets(self):
        ...
        self.button_gray_filter.grid(row=5, column=0, padx=5, pady=3, ipadx=10)
        ...
        self.Coloured_dark_letters_button.grid(row=5, column=4, padx=5, pady=3, ipadx=10)
        
        self.Gray_dark_letters_button = Button(
            self.tool_bar, text="ToGray_dark_letters", command=self.applyGray_dark_letters)
        self.Gray_dark_letters_button.grid(row=5, column=5, padx=5, pady=3, ipadx=10)

        self.Coloured_text_button = Button(
            self.tool_bar, text="ToColoured_text", command=self.applyColoured_text)
        self.Coloured_text_button.grid(row=5, column=6, padx=5, pady=3, ipadx=10)

        self.Dark_cards_button = Button(
            self.tool_bar, text="ToDark_cards", command=self.applyDark_cards)
        self.Dark_cards_button.grid(row=5, column=7, padx=5, pady=3, ipadx=10)

        self.Dark_dominos_button = Button(
            self.tool_bar, text="ToDark_dominos", command=self.applyDark_dominos)
        self.Dark_dominos_button.grid(row=5, column=8, padx=5, pady=3, ipadx=10)

        # applyColoured_letters
        # applyGray_letters
        # applyDark_letter
        # applyColoured_dark_letters
        # applyGray_dark_letters
        # applyColoured_text
        # applyDark_cards
        # applyDark_dominos
    ...
    def applyColoured_text(self):
        try:
            self.filters.coloured_text(
                                    (self.main_image.load())
                                    ,(self.main_image.size[0])
                                    , (self.main_image.size[1]),"Lorem Ipsum")
        except ValueError as error:
            showerror(title='Error', message=error)
